    ...
    for url in urls:
        try:
            ...
            if response.ok:
                ...
                response_hash['response'][url]['response_time_in_ms']=response.elapsed.total_seconds()*1000
                metrics['status'].labels(url).set(1)
                metrics['response_time'].labels(url).set(response.elapsed.total_seconds()*1000)
            else:
